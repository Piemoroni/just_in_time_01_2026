...
## 6.1 Ferramentas e Ambiente de Testes
...
1. Abrir o Insomnia, Postman ou outra ferramenta de testes.
...
6. Verificar o código de resposta retornado pela API.
...
**Entrada:**
`POST /usuarios/cadastrar` com JSON:
...
**Entrada:**
`POST /produtos/cadastrar` com JSON:
...
10. Se necessário, verificar o registro no banco de dados.
...
**Entrada:**
`POST /produtos/cadastrar` enviando custo negativo:
...
8. Também é possível validar o resultado diretamente na interface de produtos.
...
**Entrada:**
`POST /producao/registrar` com JSON:
...
5. Informar o `usuarioId` e `produtoId` válidos.
6. Selecionar o tipo de movimentação `FABRICADO`.
7. Informar a quantidade `20`.
8. Enviar a requisição.
9. Consultar novamente o estoque do produto.
10. Conferir se a movimentação foi registrada no banco.
...
**Resultado Esperado:**
Status `201 Created`, decremento no estoque e campo de `alerta` retornado na resposta da requisição. A interface também deve informar que o estoque está abaixo do mínimo configurado.
